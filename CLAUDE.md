...
- **Two themes, and a theme is the whole appearance, not a palette.** `:root` carries "Papier" (ivory/brass, queue as a vertical rail in the right margin); `[data-thema="wueste"]` carries "Desert Rose" (sand/burgundy, queue as a horizontal engraved scale under the cover, cover in a passepartout, centred column, filled play disc). Both come from `mockups/faden-cover.html` (E and F).
