...
- **Settings** are one object in `localStorage` under `musiklib:einstellungen` (`thema`, `akzent`, `leiste`), not one key per option, so later options need no new key. `applySettings()` is the single place that writes state to the DOM; it validates every value through `gewaehlt()` and falls back to the default, so a hand-edited or outdated key can't leave the UI in a state with no way out. It also updates the `theme-color` meta — otherwise the iPhone paints an ivory strip above the sand.
...
- Session keys are the ones the other two surfaces use (`musiklib:session`, `volume`, `muted`, `shuffle`) in **exactly the same shape**; `musiklib:layout` and `musiklib:repeat` are this page's own and are ignored elsewhere.
